 ...
 * rd = rs ^ rt;
 */
public class BitwiseXORReg extends ThreeAddressInstruction {

	private BitwiseXORReg(VirtualRegister rd, IRRegister rs, IRRegister rt) {
		this.rd = rd.clone();
		this.rs = rs.clone();
		this.rt = rt.clone();
	}
	...
	public static IRInstruction getExpression(VirtualRegister rd, IRRegister rs, IRRegister rt) {
		...
		return new BitwiseXORReg(rd, rs, rt);
	}
