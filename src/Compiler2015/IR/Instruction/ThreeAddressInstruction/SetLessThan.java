 ...
 * rd = rs < rt
 */
public class SetLessThan extends ThreeAddressInstruction {

	protected SetLessThan(VirtualRegister rd, IRRegister rs, IRRegister rt) {
		this.rd = rd.clone();
		this.rs = rs.clone();
		this.rt = rt.clone();
	}
	...
	public static IRInstruction getExpression(VirtualRegister rd, IRRegister rs, IRRegister rt) {
		...
		if (rs instanceof ImmediateValue)
			return new SetGreaterThan(rd, rt, rs);
