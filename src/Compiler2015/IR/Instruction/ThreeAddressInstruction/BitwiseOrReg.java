 ...
 * rd = rs | rt
 */
public class BitwiseOrReg extends ThreeAddressInstruction {

	private BitwiseOrReg(VirtualRegister rd, IRRegister rs, IRRegister rt) {
		this.rd = rd.clone();
		this.rs = rs.clone();
		this.rt = rt.clone();
	}
	...
	public static IRInstruction getExpression(VirtualRegister rd, IRRegister rs, IRRegister rt) {
		...
		return new BitwiseOrReg(rd, rs, rt);
	}
